...
| PC20 | The backup summary uses human-readable, privacy-safe German state text. It explains pending camera matching, SSD redundancy, informational safe-to-clear and automatic-work blockers without exposing source metadata or claiming completion. | **Reopened 2026-09-26 from target evidence:** after a recovered session reached a no-work/review outcome, the old advisory `PLAN_LOOKUP` presentation remained visible. `MainActivity` now clears the transient progress area whenever the service publishes a terminal no-work decision; dispatcher job-materialization loss now publishes `SOURCE_CHANGED` rather than retaining planning. `AutomaticTransferUiStatePolicyTest` plus the full 524-test JVM/debug checkpoint PASS. Focused target recheck remains required. | SOFTWARE_PROVEN_HARDWARE_PENDING |
