...
|---|---|---|---|
| PC01 | Known-camera selection respects saved MAC matching, explicit stop and replacement epochs. | **Repaired 2026-09-25:** a fresh launcher with the Pocket initially off now gets eight bounded, epoch-fenced scan windows for a saved non-stopped camera; manual Rescan invalidates pending launcher callbacks. `CameraStartupDiscoveryPolicyTest`, recovery/resource tests and full JVM/debug build **486 tests, 0 failures/errors** PASS. Physical validation is consolidated in HD01 rather than treated as an open software item. | PASS |
