...
| PC18 | Automatic camera-transfer dispatch is owned by the service/coordinator rather than the Activity; UI only observes durable progress and cannot be required to remain alive for a trusted automatic plan to execute. | **Reopened 2026-09-25:** target S25 proves automatic connect, revalidation and visible durable plan projection, but reports `download=2` while no automatic download begins because `plan complete=false; reason=IDENTITY_UNRESOLVED; current-unresolved=0; history-unresolved=3`. The block is fail-closed, but the product requirement needs an audit: decide and test whether proven safe current candidates may execute while historical ambiguity still blocks all completeness/redundancy/cleanup claims. No relaxation of identity, verification or completion invariants is authorized. | IN_PROGRESS |
