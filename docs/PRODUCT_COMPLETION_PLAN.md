...
| PC20 | The backup summary uses human-readable, privacy-safe German state text. It explains pending camera matching, SSD redundancy, informational safe-to-clear and automatic-work blockers without exposing source metadata or claiming completion. | **Refined 2026-09-26 from target feedback:** the diagnostic four-field line was replaced by one actionable, safe sentence. A terminal incomplete list now says `Kamera-Sicherung wartet auf vollständige Dateiliste`, not that an operation is still running. Active transfer copy names transfer plus the following integrity check. `BackupStatusCopyTest` (8) passes; target readability remains in consolidated physical evidence. | SOFTWARE_PROVEN_HARDWARE_PENDING |
