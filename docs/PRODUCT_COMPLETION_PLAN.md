...
| PC20 | The backup summary uses human-readable, privacy-safe German state text. It explains pending camera matching, SSD redundancy, informational safe-to-clear and automatic-work blockers without exposing source metadata or claiming completion. | **Refined again 2026-09-26 from target feedback:** the state is a dedicated card matching the camera-status card, not a small helper line. A terminal incomplete list says `Kameraliste noch nicht vollständig – deshalb keine Übertragung`; active service-owned work displays preparation/transfer/integrity progress inside the same card. `BackupStatusCopyTest` (9) and full JVM/debug checkpoint **531 tests, 0 failures/errors** PASS; target readability remains in consolidated physical evidence. | SOFTWARE_PROVEN_HARDWARE_PENDING |

| PC20-TRANSITION | Every changed visible backup/progress state has deterministic active, progress, successful-terminal, fail-closed-terminal and stale/replacement callback coverage. Progress is shown only during active service-owned work and has a bounded success dismissal. | **Audited 2026-09-26:** `UI_TRANSITION_AUDIT.md` maps every backup/progress lifecycle row. `AutomaticTransferUiStatePolicyTest` covers all dispatcher decisions, bounded completion and stale-progress terminal dominance; `BackupProjectionNotifierTest` proves immediate durable invalidation after lifecycle observer registration; `BackupStatusCopyTest` covers user-facing terminal summaries. Full JVM/debug checkpoint: **531 tests, 0 failures/errors**. | SOFTWARE_PROVEN_HARDWARE_PENDING |
