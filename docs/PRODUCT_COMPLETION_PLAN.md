...
| PC20 | The backup summary uses human-readable, privacy-safe German state text. It explains pending camera matching, SSD redundancy, informational safe-to-clear and automatic-work blockers without exposing source metadata or claiming completion. | Atomic projection and current-vs-historical inventory separation are software-proven. The target text remains conservative until terminal pagination is physically confirmed; visible service states now distinguish preparing, safe writer handoff, transfer, receipt update and review without asserting ledger truth. | SOFTWARE_PROVEN_HARDWARE_PENDING |
