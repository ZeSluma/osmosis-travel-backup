...
| PC20 | The backup summary uses human-readable, privacy-safe German state text. It explains pending camera matching, SSD redundancy, informational safe-to-clear and automatic-work blockers without exposing source metadata or claiming completion. | **Refined 2026-09-26 from target feedback:** the diagnostic four-field line was replaced by one actionable, safe sentence. Active transfer/source-change copy still wins to match the progress bar; incomplete inventory remains explicit; historical uncertainty becomes e.g. `3 ältere Dateien brauchen Aufmerksamkeit`; a complete local copy with no SSD becomes `Lokal gesichert · SSD-Kopie steht noch aus`. `BackupStatusCopyTest` and full 526-test JVM/debug checkpoint PASS. Target readability remains in the consolidated physical evidence. | SOFTWARE_PROVEN_HARDWARE_PENDING |
