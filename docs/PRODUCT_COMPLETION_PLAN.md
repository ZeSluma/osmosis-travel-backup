...
| PC20-TRANSITION | Every changed visible backup/progress state has deterministic active, progress, successful-terminal, fail-closed-terminal and stale/replacement callback coverage. Progress is shown only during active service-owned work and has a bounded success dismissal. | **Reopened 2026-09-26:** `AutomaticTransferUiStatePolicyTest` (3) now asserts planning/writer wait/transfer, bounded success acknowledgement, and no progress for review/source-change/no-work. `docs/EXECUTION_POLICY.md` makes the complete transition matrix mandatory before hardware. Broader matrix audit remains open. | IN_PROGRESS |
