...
|---|---|---|---|
| PC01 | Known-camera selection respects saved MAC matching, explicit stop and replacement epochs. | **New target finding 2026-09-25:** if the app starts while the Pocket is off, its one 4-second launcher scan expires before the Pocket advertises; turning the camera on afterwards did not begin another scan. The camera-first/app-second order connected successfully. Repair adds eight bounded, epoch-fenced launcher-discovery scans for a saved non-stopped camera, then honestly becomes user-action-required; manual Rescan invalidates pending launcher callbacks. `CameraStartupDiscoveryPolicyTest`, recovery/resource tests and full JVM/debug build **486 tests, 0 failures/errors** PASS. The one focused target retest is pending. | IN_PROGRESS |
