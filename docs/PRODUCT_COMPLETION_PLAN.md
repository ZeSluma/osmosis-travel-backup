...
| PC18 | Automatic camera-transfer dispatch is owned by the service/coordinator rather than the Activity; UI only observes durable progress and cannot be required to remain alive for a trusted automatic plan to execute. | **Repaired and target-retested 2026-09-19:** target S25 initially showed a populated connected grid with `backupSummary` empty. Root cause: `releaseTransport()` correctly cleared the prior source association but selection installed it before teardown, so the service publisher had no association; strict-transfer capability was also set after first service publication. Repair installs both only after release and before GATT/datalink; blank UI now renders an honest awaiting-projection state. `CameraSessionResourcesTest`, `AutomaticTransferDispatchPolicyTest`, `CameraDatalinkCoordinatorTest` PASS; full JVM/debug build **472 tests, 0 failures/errors**. In-place target retest displayed `Camera sync: pending`, `Redundancy: pending`, `Safe to clear: no`, and `plan complete=false; reason=IDENTITY_UNRESOLVED; current-unresolved=0; history-unresolved=3; download=2; verify=3; revalidate=1; review=0`. The blocked writer is expected fail-closed behavior, not a false completion. | PASS |
