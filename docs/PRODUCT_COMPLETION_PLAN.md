...
| PC20 | The backup summary uses human-readable, privacy-safe German state text. It explains pending camera matching, SSD redundancy, informational safe-to-clear and automatic-work blockers without exposing source metadata or claiming completion. | `BackupStatusCopyTest` plus `ProductStateMatrixTest` prove copy selection internally. **Target defect 2026-09-25:** after a trusted seven-file inventory, correct per-file states render but the summary remains stuck at `Kameraliste wird noch geprüft` instead of showing the durable historical-identity explanation. Diagnose/fix the observer refresh race, add a regression, then re-audit. | OPEN |
