...
| PC18 | Automatic camera-transfer dispatch is owned by the service/coordinator rather than the Activity; UI only observes durable progress and cannot be required to remain alive for a trusted automatic plan to execute. | **Repair 2026-09-25; target execution pending:** S25 showed `download=2` but no automatic writer because three historical unresolved identities made the global plan incomplete. `SnapshotCompletenessPolicy` now makes a fresh `DOWNLOAD` eligible only when the *current* observation has full page/store/member/stability coverage and zero current unresolved identities. Historical ambiguity still keeps `enumerationComplete=false` and continues to block Camera Sync Complete, redundancy and Safe-to-Clear. `AutomaticBackupPlanTest`, `SnapshotCompletenessPolicyTest`, `AutonomousBackupRuntimeTest`, `AutomaticTransferDispatchPolicyTest`, and full `:app:testDebugUnitTest :app:assembleDebug` passed (**479 tests, 0 failures/errors**). A controlled S25 observation must still prove service-owned dispatch begins for the two safe current candidates without false completion. | IN_PROGRESS |
