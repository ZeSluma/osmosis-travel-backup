...
| R-041 Diagnostik | **Teilweise umgesetzt am 2026-09-25:** `DiagnosticEventStore` speichert Session-, Transfer-, Storage- und User-Aktion-Zustände app-privat, auf sieben Tage/10 MiB begrenzt, als ausschließlich erlaubnislistenbasierte Codes und kann nur diese Repräsentation explizit exportieren. 3 Store-Tests sowie Session-/Transfer-Regressionsläufe sind PASS. Offen: End-to-End-Sinknachweis, UI-Export und Ablösung bzw. harte Begrenzung des alten optionalen Dateilogs. | Alle Sinkpfade end-to-end prüfen bzw. den Altfallback sicher begrenzen; Tests für Transfer/Storage/Export-UI und reale Reason-State-Beobachtung ohne sensible Ausgabe. |
