...
| R-041 Diagnostik | **Teilweise umgesetzt am 2026-09-25:** `DiagnosticEventStore` speichert Session-, Transfer-, Storage- und User-Aktion-Zustände app-privat, auf sieben Tage/10 MiB begrenzt, als ausschließlich erlaubnislistenbasierte Codes und kann nur diese Repräsentation explizit exportieren. Der separate Verbose-Pfad ist nun ebenfalls app-privat, explizit, auf 30 Minuten/10 MiB begrenzt und startet nach Prozessneustart nicht wieder. 500 JVM-Tests, 0 Fehler, Debug-Build PASS. Offen: End-to-End-Sinknachweis und UI-Export des typisierten Kanals sowie reale Reason-State-Beobachtung ohne sensible Ausgabe. | Alle Sinkpfade end-to-end prüfen; UI für den expliziten Export des typisierten Kanals ergänzen; reale Reason-State-Beobachtung ohne sensible Ausgabe. |
