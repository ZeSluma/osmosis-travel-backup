...
## GATE-9 — SAFE CAMERA CLEANUP

Accepted via ADR0006; NOT_TESTED and no implementation authorization. Dependencies: GATE-4 PASS with all G2/G3/G7 prerequisites, plus at least one required secondary-destination gate sufficient for policy (G5 OR G6 by default; both only if policy explicitly requires both). GATE-8 is not required. Require reviewed strong source/storage identity and group collateral effects, current two-domain replica proof, exhaustive pre/post enumeration, one explicit scope-bound destructive confirmation (enhanced mode optional/OFF), immediate safety revocation on changed evidence, durable partial-operation recovery under ADR0006, security of every destructive entry point, CL01-CL12 and S25/Pocket HIL using separately authorized disposable recordings. No real destructive use before dedicated acceptance and explicit authorization. Protocol ambiguity leaves the action unavailable. No format fallback. Historical gate records stay intact.
