...
KA08 — Wake/recovery boundary: investigate [CAMERA_UNREACHABLE_UNTIL_POWER_CYCLE](../evidence/GATE-2/2026-09-17_hardware/CAMERA_UNREACHABLE_UNTIL_POWER_CYCLE.md) separately. Green light off and failed Rescan followed by recovery after user power-cycle do not establish full power-off, standby or AP/BLE shutdown. Verify actual Pocket4P BLE remote-wake support, the wake command Osmosis sends and its response, distinguishability of deep standby/full power-off, bounded automatic recovery and evidence-backed `USER_ACTION_REQUIRED: POWER_ON_CAMERA`. Preserve UNKNOWN where distinctions cannot be measured. KA08 NOT_TESTED; no wake implementation or setting change during G2. Do not press Back to Live View without a test-specific need.

Separate observation: [IDLE_OR_STANDBY_LIKE_STATE with green LED ON](../evidence/GATE-2/2026-09-17_hardware/NEW_RECORDING_TIMESTAMP_EVIDENCE.md), fresh Rescan discovery/connection succeeded and new media enumerated. Exact power/session state UNCONFIRMED. Do not merge with LED-off unreachable-until-power-cycle, connected-but-empty or saved-entry failure; no remote wake capability proven by successful discovery alone.
