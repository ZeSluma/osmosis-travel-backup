...
Latest target reconciliation supersedes the initial missing-locator observation below: schema3 links both protected files as metadata-only LOCAL_PRESENT_UNVERIFIED candidates; repeated enumeration/restart retains identities, paths and local references without duplicates. Source integrity remains unverified. Separate [CONNECTED_BUT_MEDIA_ENUMERATION_EMPTY](evidence/GATE-2/2026-09-17_hardware/CONNECTED_BUT_MEDIA_ENUMERATION_EMPTY.md) and [CAMERA_UNREACHABLE_UNTIL_POWER_CYCLE](evidence/GATE-2/2026-09-17_hardware/CAMERA_UNREACHABLE_UNTIL_POWER_CYCLE.md) observations extend RISK-016/019/044 investigation. Zero-media snapshot stayed INCOMPLETE with historical candidates intact; after user power-cycle the same2 assets returned. Do not infer true empty source, pure Wi-Fi failure, sleep or full power-off. G7 KA08 must establish BLE wake support/actual command correctness, distinguishability of deep standby/full power-off, automatic recovery limits and evidence-backed USER_ACTION_REQUIRED: POWER_ON_CAMERA. Root cause and wake capability remain UNCONFIRMED/NOT_TESTED.
