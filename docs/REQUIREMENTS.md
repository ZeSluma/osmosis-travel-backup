...
**R-039 Explicit snapshot cleanup:** future user-initiated, explicitly confirmed DELETE_VERIFIED_SOURCE_SNAPSHOT only; revalidate source/camera/storage and replicas immediately before execution, retain new/unverified media, journal per-asset results, never blindly retry uncertain handles, verify all intended absent and retained assets present before CAMERA CLEANUP COMPLETE. No format, automatic initiation or early-gate destructive implementation. One explicit destructive confirmation is the default; enhanced confirmation is optional/OFF. Live safety and exact plan/proof-generation binding revoke approval on material changes. ADR0006 permits bounded same-live-operation continuation only after unchanged-safety reconciliation; process death/camera restart or changed evidence require one new confirmation of the remaining scope. See VERIFIED_SNAPSHOT_CLEANUP and accepted GATE-9.
